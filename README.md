...
### ***NOTE:From step-1 to step-9 You have to do it on all the servers
...
```
### Delete all the lines in the config.toml file
...
```
### This step do it only on  master server
...
```
#### Execute this command as root user on all the worker nodes, Please replace with your token
...
```
### Execute from master server as ubuntu user
### Step 12:
```bash
#Apply the CNI YAML
kubectl get nodes
